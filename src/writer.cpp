...
node->next; node = node->next) {
    ...
    char temp[20]={NULL,};
    std::strncpy(temp,node->surface,node->length);

    if(node->next != NULL && std::regex_match(temp,re)){
	char temp2[20]={NULL,};
	std::strncpy(temp2, node->next->surface, node->next->length);
	if(!std::strcmp(temp2,"세")){
		node = node->next;
		*os << temp << temp2 << '\t' << "NNG,*,*,*,*,*,*,*"<<'\n';
		continue;
	}
